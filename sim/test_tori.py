...
async def test_a(dut):
    ...
    dut.rst_in.value = 1
    dut.dda_data_valid_in.value = 1 # // Heba input // in - data on the sender_axis_tdata signal is valid and can be written into the FIFO
    dut.dda_data_ready_out.value = 1 # in - FIFO in is ready to accept data from the sender
    await ClockCycles(dut.pixel_clk_in, 5)
    dut.rst_in.value = 0



    hcount_ray, stepX, stepY, rayDirX, rayDirY, deltaDistX, deltaDistY, posX, posY, sideDistX, sideDistY
...
def is_runner():
    ...
    sources = [
        ...
        proj_path / "hdl" / "dda_fifo_wrap.sv",
        proj_path / "hdl" / "divider.sv",
        proj_path / "hdl" / "ray_calculations.sv",
        proj_path / "hdl" / "tori_testing.sv",
    ]
